...
_CHEAPEST_MODELS = [
    (InstrumentName.CLAUDE, "haiku"),
    (InstrumentName.CODEX, "gpt-5.4-mini"),
]
...
@router.post(
    ...
    description=(
        "Uses a musician to AI-generate test scenario content (story + QA pairs). "
        "Optionally specify `provider` and `model` to target a specific musician; "
    ...
    responses={503: {"description": "No cheap model musician is available.", "model": ErrorDetail}},
)
async def test_generate_scenario(request: Request, body: TestGenerateRequest) -> TestGenerateResponse:
    ...
    if musician is None:
        for instrument, model in _CHEAPEST_MODELS:
            candidate = await orchestra.acquire_musician(instrument, model)
    ...
    if musician is None:
        raise HTTPException(
            status_code=503,
            detail="No cheap model musician is currently available. Ensure haiku or gpt-5.4-mini musicians are running.",
        )
